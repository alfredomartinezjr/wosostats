...
Want to log stats on your own and help out? Please do! Read more [here](https://github.com/amj2012/wosostats/blob/master/resources/how-to-log-stats.md) and email me at alfredom790@gmail.com if you're interested. To learn about the action definitions that are used to log stats and used in this analysis, read more [here.](https://github.com/amj2012/wosostats/blob/master/resources/definitions.md)
...
### Shots Under Pressure
*I define what makes a shot (and a pass, for that matter) as being "under pressure" [here.](https://github.com/amj2012/wosostats/blob/master/resources/definitions.md)*
...
t2 <- createTable(c("recoveries", "Def 3rd", "Mid 3rd", "Opp 3rd",
                    "O6", "O18", "O3L", "O3C", "O3R","OM3L", "OM3C", 
                   "OM3R", "DM3L", "DM3C", "DM3R", "D3L", "D3C", "D3R", 
                   "D18", "D6"), "poss.location", t)

## Fill in blanks
t2$recoveries <- rowSums(t2[,5:20])
t2[,"Def.3rd"] <- rowSums(t2[,c("D3L", "D3C", "D3R", "D18", "D6")])
t2[,"Mid.3rd"] <- rowSums(t2[,c("OM3L", "OM3C", "OM3R", "DM3L", "DM3C", "DM3R")])
t2[,"Opp.3rd"] <- rowSums(t2[,c("O6", "O18", "O3L", "O3C", "O3R")])
t2 <- t2[,1:4]
...
t <- createDataFrame(c("clearances", "balls.shielded", "blocks"), "def.action", d)
t <- t[,c("event","time", "def.position","def.team","def.player","def.action","def.location", "def.player.disciplinary","def.notes")]

names(t) <- c("event", "time", "position","team", "poss.player", "player.event", "location", 
              "def.player.disciplinary", "def.notes")

t2 <- createTable(c("blocks", "clearances", "balls.shielded"), "player.event", t)
...
names(t2) <- c("Won", "Lost", "Caught", "Punched Away", "Parried", "Collected", "Fouled", "Crosses Faced", "Corners Faced", "FK Faced")
